...
一个帮助用户重新回到真实对话、选择自己要说的话，并与 AI 模拟的“对方”连续练习对话的网页应用。对方可以是任何性别。

## 已包含

- 11 步问卷：关系、场景、对方的话、说话方式、可选聊天样本、当时状态、原回答、感受、核心意图、期待与边界、语气与长度
- 可粘贴聊天记录或导入 TXT、MD、JSON、CSV、LOG 文本文件，帮助 AI 提炼对方的表达与反应规律
- 记录对方的口头禅、沟通意愿与冲突反应，让回复更接近具体人物而不是通用客服语气
- 3 个可编辑的开场草稿：核心表达、更温和、边界更清楚
- 问卷结束后进入聊天框，由用户逐句决定自己要说什么
- AI 根据人物声音、当时情绪、沟通意愿和完整对话记录，逐轮生成“对方的一种可能回复”
- 对方可以迟疑、误解、反驳、回避、冷淡或结束对话，不会默认每次都完美理解用户
- 明确标注模拟回复不代表真实人物的想法
...
- 聊天参考样本是选填项，只支持文本内容，单个文件最大 200 KB；浏览器最多保留前 16,000 个字符。
- 导入前应删除真实姓名、电话、地址、账号、身份证件和其他不必要的隐私。应用不会自动保证已经完全脱敏。
- 配置 API Key 后，生成开场草稿时，问卷内容和原始聊天参考样本会由服务端发送到 OpenAI API 一次，用来提炼简短的表达与反应画像。
- 后续逐轮对话只发送这份提炼后的画像和当前练习内容，不重复发送整份原始聊天参考样本。
- 这里的“参考样本”只是本次练习的上下文，不会在应用中创建微调任务，也不会永久训练一个专属模型。
- OpenAI Responses API 请求使用 `store: false`。
- 应用自身不把问卷、聊天参考样本、提炼画像或练习对话写入数据库。
